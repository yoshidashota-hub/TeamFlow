...
import { createTaskSchema, updateTaskSchema, deleteTaskSchema, getTasksSchema } from "../types";
import { TaskService } from "./task.service";

export const taskRouter = router({
  // タスク一覧取得（検索・フィルター対応）
  list: protectedProcedure.input(getTasksSchema).query(async ({ ctx, input }) => {
    return TaskService.list(ctx.session.user.id!, input);
  }),

  // タスク取得（ID指定）
  getById: protectedProcedure.input(deleteTaskSchema).query(async ({ ctx, input }) => {
